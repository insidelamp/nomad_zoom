...
STUN 서버는 컴퓨터가 공용 ip 주소를 찾게해줌 
내폰에서 와이파이를 끄고데 데이터로 접속시 폰이 컴퓨터를 찾지못함 
peer to peer 연결을 하고있기때문에 서로를 찾아야함 
strn 서버는 어떤것을 request 하면 인터넷에서 사용자가 누군지를 알려주는 서버임  ( 사용자의 공용 ip를 알려줌)
makeConnection 함수에 사용하려는 stun 서버의 리스트를 추가해주면됨
전문적으로는 직접만들어야하지만 구글이 무료로 제공하는 서버를 사용하는거로할것임 
webRTC 를 사용한 실제 서비스나 전문적이 뭔가를 만들고 싶다면  내소유의 stun 서버가 필요함 

stun 서버는 장치에 공용주소를 알려주는 서버임 나의 프로젝트를 만들경우 서버가있어야함 
